...
if __name__ == '__main__':
    ...
    if args.category is not None:
        shape_info_paths = [
            ...
            if args.category.lower() in p.stem.lower()
        ]
        Log.info(f"Path filtering({args.category}) applied: {len(all_json_paths)} -> {len(shape_info_paths)}")
    else:
    ...
    for shape_json_path in shape_info_paths:

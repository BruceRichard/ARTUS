...
from .geometry import DiffusionModelGeometry
...
class Diffusion(TransArticulatedBaseModule):
    def __init__(self, config):
        ...
        if config.get('flow_matching', {}).get('enabled', False):
            Log.info("Using Flow Matching (Dual Loss Hybrid Training)")
            self.model = DiffusionModelGeometry(diffusion_core, config)
        else:
            Log.info("Using standard DDPM training")
